...
const AdminPage = () => {
  ...
  const [activeGmPlayer, setActiveGmPlayer] = useState(null);
  ...
  const handleGmResponse = (playerName) => {
  ...
  const handlePassToAI = (playerName) => {
    db.clearPendingAction(playerName);
    loadAllData();
  };
  
  const handleContextChange = (field, value) => setEditedContext(prev => ({ ...prev, [field]: value }));
  const handleStoryChange = (index, field, value) => setEditedStory(prev => prev.map((part, i) => i === index ? { ...part, [field]: value } : part));
  const deleteStoryPart = (index) => { if(window.confirm('Hapus entri cerita ini?')) { setEditedStory(prev => prev.filter((_, i) => i !== index)); }};
  ...
  const handleSaveChanges = () => {
    ...
    try {
      ...
      const newGameContext = { ...currentSaveData.gameContext, ...editedContext, characters: newCharacters };
      const newSaveData = { ...currentSaveData, gameContext: newGameContext, storyParts: editedStory };
  ...
  const getCountdown = (timestamp) => Math.ceil(Math.max(0, GM_TIMEOUT - (Date.now() - timestamp)) / 1000);
