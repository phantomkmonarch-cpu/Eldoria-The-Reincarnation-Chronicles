...
import type { GameState, StoryPart, GameContext, SaveData, ChapterRule } from '../types';
...
const seasonChapters: Record<number, Record<number, ChapterRule>> = {
  1: {
    1: {
...
const getInitialGameContext = (playerName: string): GameContext => ({
...
export const useGame = () => {
  ...
  const gmTimeoutRef = useRef<number | null>(null);
  const loggedInPlayerName = useRef<string | null>(null);

  const saveGame = useCallback((pName: string, context: GameContext, story: StoryPart[]) => {
  ...
  const addStoryPart = useCallback((part: Omit<StoryPart, 'timestamp'>) => {
    const newPart: StoryPart = { ...part, timestamp: new Date().toISOString() };
    setGameState(prev => {
  ...
  const callAI = useCallback(async (actionText: string) => {
    if (!gameState.gameContext) return;
